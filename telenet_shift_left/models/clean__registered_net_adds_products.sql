...
, product_mapping as (
    ...
    from exclude_migrations_feb_2025 emf
    left join {{ source('product_offering_explorer', 'd_mts_enrollment_product') }} as pp
        on pp.star_enrollment_product_id = emf.star_product_id
    left outer join {{ source('product_offering_explorer', 'd_date') }} as d
        on d.star_date_id = emf.star_transaction_date_id
)
